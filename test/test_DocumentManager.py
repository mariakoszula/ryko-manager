...
def prepare_fruit_data(program_id, school, weeks):
    ...
    return (fruit_all, max_kids_perWeeks_fruitVeg, set([apple, pear, plum, strawberry, juice]))
...
def prepare_veg_data(program_id, school, weeks):
    ...
    max_kids_perWeeks_fruitVeg = DatabaseManager.get_maxKids_perWeek(program_id, school, ProductType.FRUIT_VEG, weeks)
    return (veg_all, max_kids_perWeeks_fruitVeg, set([carrot, radish, pepper, tomato, kohlrabi]))
...
def prepare_dairy_data(program_id, school, weeks):
    ...
    return (dairy_all, max_kids_perWeeks_milk, set([milk, yoghurt, kefir, cheese]))
...
def test_product_info():
    ...
    program_id = 5
    school_id = 1
    assert (DatabaseManager.get_product_amount(program_id, school_id, ProductName.APPLE, weeks) == 365)
    assert (DatabaseManager.get_product_amount(program_id, school_id, ProductName.MILK, weeks) == 910)
