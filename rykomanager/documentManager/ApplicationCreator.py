...
class ApplicationCreator(DocumentCreator, DatabaseManager):
    ...
    def _generate_5(self):
        ...
        self.document.merge(
            ...
            week_date_1=DatabaseManager.get_dates(self.program_id, 1) if self.summary.is_first else "-",
            week_date_2=DatabaseManager.get_dates(self.program_id, 2) if self.summary.is_first else "-",
            week_date_3=DatabaseManager.get_dates(self.program_id, 3) if self.summary.is_first else "-",
            week_date_4=DatabaseManager.get_dates(self.program_id, 4) if self.summary.is_first else "-",
            week_date_5=DatabaseManager.get_dates(self.program_id, 5) if self.summary.is_first else "-",
            week_date_6=DatabaseManager.get_dates(self.program_id, 6) if self.summary.is_first else "-",
            week_date_7=DatabaseManager.get_dates(self.program_id, 7) if not self.summary.is_first else "-",
            week_date_8=DatabaseManager.get_dates(self.program_id, 8) if not self.summary.is_first else "-",
            week_date_9=DatabaseManager.get_dates(self.program_id, 9) if not self.summary.is_first else "-",
            week_date_10=DatabaseManager.get_dates(self.program_id, 10) if not self.summary.is_first else "-",
            week_date_11=DatabaseManager.get_dates(self.program_id, 11) if not self.summary.is_first else "-",
            week_date_12=DatabaseManager.get_dates(self.program_id, 12) if not self.summary.is_first else "-",
            fuitVeg_week1=str(DatabaseManager.get_maxKids_perWeek(self.program_id, self.school.id, ProductType.FRUIT_VEG, (1,1)) if self.summary.is_first else "-"),
            fuitVeg_week2=str(DatabaseManager.get_maxKids_perWeek(self.program_id, self.school.id, ProductType.FRUIT_VEG, (2,2)) if self.summary.is_first else "-"),
            fuitVeg_week3=str(DatabaseManager.get_maxKids_perWeek(self.program_id, self.school.id, ProductType.FRUIT_VEG, (3,3)) if self.summary.is_first else "-"),
            fuitVeg_week4=str(DatabaseManager.get_maxKids_perWeek(self.program_id, self.school.id, ProductType.FRUIT_VEG, (4,4)) if self.summary.is_first else "-"),
            fuitVeg_week5=str(DatabaseManager.get_maxKids_perWeek(self.program_id, self.school.id, ProductType.FRUIT_VEG, (5,5)) if self.summary.is_first else "-"),
            fuitVeg_week6=str(DatabaseManager.get_maxKids_perWeek(self.program_id, self.school.id, ProductType.FRUIT_VEG, (6,6)) if self.summary.is_first else "-"),
            fuitVeg_week7=str(DatabaseManager.get_maxKids_perWeek(self.program_id, self.school.id, ProductType.FRUIT_VEG, (7,7)) if not self.summary.is_first else "-"),
            fuitVeg_week8=str(DatabaseManager.get_maxKids_perWeek(self.program_id, self.school.id, ProductType.FRUIT_VEG, (8,8)) if not self.summary.is_first else "-"),
            fuitVeg_week9=str(DatabaseManager.get_maxKids_perWeek(self.program_id, self.school.id, ProductType.FRUIT_VEG, (9,9)) if not self.summary.is_first else "-"),
            ...
            milk_week4=str(DatabaseManager.get_maxKids_perWeek(self.program_id, self.school.id, ProductType.DAIRY, (4,4)) if self.summary.is_first else "-"),
            milk_week5=str(DatabaseManager.get_maxKids_perWeek(self.program_id, self.school.id, ProductType.DAIRY, (5,5)) if self.summary.is_first else "-"),
            milk_week6=str(DatabaseManager.get_maxKids_perWeek(self.program_id, self.school.id, ProductType.DAIRY, (6,6)) if self.summary.is_first else "-"),
            milk_week7=str(DatabaseManager.get_maxKids_perWeek(self.program_id, self.school.id, ProductType.DAIRY, (7,7)) if not self.summary.is_first else "-"),
            milk_week8=str(DatabaseManager.get_maxKids_perWeek(self.program_id, self.school.id, ProductType.DAIRY, (8,8)) if not self.summary.is_first else "-"),
            milk_week9=str(DatabaseManager.get_maxKids_perWeek(self.program_id, self.school.id, ProductType.DAIRY, (9,9)) if not self.summary.is_first else "-"),
    ...
    def _generate_5a(self):
        ...
        self.document.merge(
            ...
            school_address=self.school.address,
            city=self.school.city,
            weeks=DatabaseManager.str_from_weeks(DatabaseManager.get_weeks(self.program_id), self.weeks),
