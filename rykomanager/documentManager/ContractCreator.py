...
class ContractCreator(DocumentCreator, DatabaseManager):
    ...
    main_contract_dir = path.join(cfg.output_dir_main, cfg.annex_folder_name, cfg.contract_dir_name)
    ...
    def __init__(self, school, program_id):
        ...
        self.school = school
        self.contract_no = DatabaseManager.get_next_contract_no(self.program.id)
    ...
    def create(self, contract_date=None):
        ...
        if DatabaseManager.get_contract(self.school.id, self.program.id):
            app.logger.error("[%s] Contract already exists [%s, %s]. Only modifying is possible", __class__.__name__,
    ...
    def generate(self):
        self.document.merge(
            ...
            semester=DatabaseManager.get_current_sem(),
        ...
        #DocumentCreator.copy_to_path(doc_contract_name, doc_contract_name_copy)
        ##DocumentCreator.generate(self, doc_contract_name_copy, False)
        #if path.exists(doc_contract_name_copy):
        #    remove(doc_contract_name_copy)
        #copyfile(path.join(self.output_directory, doc_contract_name), doc_contract_name_copy)
